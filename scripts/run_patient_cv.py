...
def parse_args():
    p = argparse.ArgumentParser(
        description="Orchestrate 5-fold Patient-level Cross-Validation"
    )
    p.add_argument(
        "--model",
        required=True,
    ...
    p.add_argument(
        "--stage",
        default="s3_transfer",
        choices=["s1_isolate", "s2_treatment", "s3_transfer"],
    )
    p.add_argument("--exp-name", default=None, help="Base experiment name prefix")
    p.add_argument(
        "--exp-dir", default="experiments", help="Parent experiment output directory"
    )
    ...
    p.add_argument(
        ...
        nargs="*",
        default=[],
        help="Dotlist overrides for config parameters",
    )
