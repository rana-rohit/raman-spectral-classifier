...
def _find_eval_results(exp_dir: Path) -> List[Path]:
    ...
    if parent_analysis_dir.exists():
        ...
        parent_recursive_matches = sorted(
            parent_analysis_dir.rglob("*_eval_results.json")
        )
...
def _save_fig(fig, path: Path, dpi: int) -> None:
    ...
    fig.savefig(
        str(pdf_path), dpi=min(dpi, 300), bbox_inches="tight", facecolor="white"
    )
    plt.close(fig)
...
def plot_confusion_matrix(
    ...
    if annot:
        if normalize:
            annot_data = np.array(
                [[str(int(v)) if v > 0 else "" for v in row] for row in cm_plot]
            )
        else:
            annot_data = np.array(
                [["" if int(v) == 0 else str(int(v)) for v in row] for row in cm_plot]
            )
    else:
        annot_data = False
    ...
    ax.set_xticklabels(
        ax.get_xticklabels(), rotation=rotation, ha="right" if rotation else "center"
    )
...
def plot_grouped_vs_spectrum(
    ...
    for split_name, data in results.get("splits", {}).items():
        metrics = data.get("metrics") or data.get("spectrum_metrics") or {}

        group_metrics = data.get("group_metrics") or data.get("patient_metrics") or {}
    ...
    ymin = min(min(spectrum_vals), min(patient_vals))

    ax.set_ylim(max(0, ymin - 0.1), 1.02)
    ax.set_ylabel("Score")
    ax.set_title(f"Patient vs Spectrum Performance\n{title}", pad=15)
    ax.legend()
...
def plot_ood_degradation(
    ...
    # Reference line
    ax.axhline(
        y=test_acc, color=PALETTE["primary"], linestyle="--", alpha=0.3, linewidth=1
    )
...
def plot_model_comparison(
    ...
    if any("patient_accuracy" in m for m in model_data):
        metrics_dict["Patient Acc."] = [
            m.get("patient_accuracy", 0) for m in model_data
        ]
...
def plot_learning_curves(
    ...
    colors = {
        "train": PALETTE["primary"],
        "val": PALETTE["secondary"],
        "source_val": PALETTE["accent"],
    }
    ...
    for split, data in split_data.items():
        ...
        if losses:
            ax.plot(
                ...
                losses,
                label=f"{split} loss",
                color=colors.get(split, PALETTE["muted"]),
                linewidth=1.8,
            )
    ax.set_xlabel("Epoch")
    ax.set_ylabel("Loss")
...
def write_summary_md(
    ...
    for split_name, data in results.get("splits", {}).items():
        ...
        lines.append(
            f"- Macro F1: **{metrics.get('f1_macro', 'N/A'):.4f}**"
            if isinstance(metrics.get("f1_macro"), float)
            else f"- Macro F1: N/A"
        )
        lines.append(
            f"- MCC: **{metrics.get('mcc', 'N/A'):.4f}**"
            if isinstance(metrics.get("mcc"), float)
            else f"- MCC: N/A"
        )
...
def _metrics_from_detailed_split(
    split_data: dict,
    ...
    if grouped_preds is not None and grouped_targets is not None:
        patient_metrics = _compute_metrics_from_labels(
            np.asarray(grouped_targets), np.asarray(grouped_preds)
        )
    else:
        ...
        if pids is None:
            ...
            patient_preds, patient_targets = _patient_vote_local(
                probs, targets, np.asarray(pids)
            )
            patient_metrics = _compute_metrics_from_labels(
                patient_targets, patient_preds
            )
    ...
    for fold_dir in fold_dirs:
        ...
        if eval_files:
            ...
            for split_name, split_data in data.get("splits", {}).items():
                ...
                if group_metrics:
                    if (
                        "precision_macro" not in group_metrics
                        and "targets" in group_metrics
                        and "predictions" in group_metrics
                    ):
        ...
        for split_name, split_data in detailed.items():
            ...
            if (fold_dir.name, split_name, "Spectrum") not in known:
                _add_metric_row(
                    ...
                    "Spectrum",
                    spectrum_metrics,
                    len(split_data.get("targets") or []),
                )
            if patient_metrics and (fold_dir.name, split_name, "Patient") not in known:
                n_patients = (
                    len(set(split_data.get("patient_ids") or []))
                    if split_data.get("patient_ids")
                    else None
                )
                _add_metric_row(
                    rows,
...
def plot_fold_metric_comparison(
    rows: List[Dict], metric: str, level: str, out_path: Path, dpi: int
) -> None:
    ...
    for idx, split in enumerate(splits):
        ...
        for fold in folds:
            match = next(
                (r for r in subset if r["fold"] == fold and r["split"] == split), None
            )
...
def plot_cross_fold_statistics(
    stats_rows: List[Dict], out_path: Path, dpi: int
) -> None:
    if not stats_rows:
        return
    preferred_split = next(
        (
            split
            for split in ["clinical_all", "test", "2018clinical", "2019clinical"]
    ...
    for ax, level in zip(axes, ["Spectrum", "Patient"]):
        subset = [
            r
            for r in stats_rows
            if r["split"] == preferred_split and r["level"] == level
        ]
        if not subset:
            ax.axis("off")
        ...
        ax.bar(
            ...
            yerr=errors,
            capsize=4,
            color=PALETTE["primary"] if level == "Spectrum" else PALETTE["accent"],
            alpha=0.88,
            edgecolor="white",
        )
        ax.set_xticks(x)
        ...
        ax.set_title(
            f"{level}-level ({_split_display(preferred_split)})", fontweight="bold"
        )
        for i, r in enumerate(subset):
            ax.text(
                i,
                r["mean"] + r["std"] + 0.02,
                _format_mean_std(r["metric"], r["mean"], r["std"]),
                ha="center",
                va="bottom",
                fontsize=8,
            )
...
def write_cross_fold_report(
    rows: List[Dict], stats_rows: List[Dict], out_path: Path
) -> None:
    _ensure_dir(out_path.parent)
    ...
    if not rows:
        ...
        for row in stats_rows:
            ...
            lines.append(
                f"- {label}: **{row['formatted']}** (min {row['min']:.4f}, max {row['max']:.4f})"
            )
...
def _bundle_from_detailed(
    fold_dirs: List[Path], split: str
        ...
        for split_name in split_names:
            ...
            preds_list.append(
                np.asarray(
                    ...
                    or np.asarray(data["probabilities"]).argmax(axis=-1)
                )
            )
...
def process_aggregated_cv_results(
    ...
    for split_name, split_data in aggregate.get("splits", {}).items():
        ...
        grouped_path = (
            subdirs["grouped"] / f"clinical_cv_grouped_vs_spectrum_{split_name}.png"
        )
        ...
        for level, cm_key, stem in [
            ...
            for normalize, tag in [(False, "raw"), (True, "normalized")]:
                path = (
                    subdirs["confusion"] / f"clinical_cv_{split_name}_{stem}_{tag}.png"
                )
        ...
        if bundle is not None:
            ...
            plot_roc_clean(
                probs,
                targets,
                labels[: probs.shape[1]],
                f"ROC - Aggregated {_split_display(split_name)}",
                roc_path,
                dpi,
            )
            if roc_path.exists():
            ...
            if split_name == "clinical_all" and len(pids) == len(targets):
                ...
                for normalize, tag in [(False, "raw"), (True, "normalized")]:
                    path = (
                        subdirs["confusion"]
                        / f"clinical_all_patient_from_probabilities_{tag}.png"
                    )
                    plot_confusion_matrix(
                        patient_targets,
                        patient_preds,
                        labels[: probs.shape[1]],
                        "Clinical All Patient-level",
                        path,
                        dpi,
                        normalize=normalize,
                    )
...
def write_domain_shift_outputs(
    metric_rows: List[Dict], subdirs: Dict[str, Path], out_root: Path, dpi: int
) -> List[str]:
    figures: List[str] = []
    ...
    rows = [
        ...
        if r["split"] in required and r["level"] in ("Spectrum", "Patient")
    ]
    ...
    levels = [
        level
        for level in ["Spectrum", "Patient"]
        if any(r["level"] == level for r in rows)
    ]
    fig, axes = plt.subplots(
        1, len(levels), figsize=(7 * len(levels), 5), squeeze=False
    )
    ...
    for ax, level in zip(axes[0], levels):
        ...
        ax.set_xticks(x)
        ax.set_xticklabels(
            [_split_display(s) for s in split_order], rotation=20, ha="right"
        )
        ...
        ax.legend(fontsize=8)
    fig.suptitle(
        "Stage 3 In-domain vs Clinical Domain Shift",
        fontsize=14,
        fontweight="bold",
        y=1.03,
    )
    ...
    for level in levels:
        test_row = next(
            (r for r in rows if r["level"] == level and r["split"] == "test"), None
        )
        clinical_row = next(
            (r for r in rows if r["level"] == level and r["split"] == "clinical_all"),
            None,
        )
        ...
        for metric in ["accuracy", "f1_macro", "mcc"]:
            if metric in test_row and metric in clinical_row:
                ...
                lines.append(
                    f"- {METRIC_DISPLAY[metric]} clinical degradation: **{gap:.4f}**"
                )
        lines.append("")
...
def plot_publication_panel(
    ...
    ax_cm.set_xticklabels(
        ax_cm.get_xticklabels(), rotation=rotation, ha="right" if rotation else "center"
    )
...
def _discover_sibling_experiments(exp_dir: Path) -> List[Dict]:
    ...
    for sibling in parent.iterdir():
        if not sibling.is_dir():
            continue
        if (
            sibling.resolve() in {exp_dir.resolve(), own_experiment.resolve()}
            or sibling.name == "analysis"
        ):
            continue
        ...
        try:
            ...
            if aggregate_path is not None:
                ...
                if spec:
                    row = {
                        "name": _load_config(sibling)
                        .get("model", {})
                        .get("name", sibling.name),
                        "dir": str(sibling),
            ...
            if not eval_files:
                eval_files = [
                    p
                    for p in sibling.rglob("*_eval_results.json")
                    if "research_plots" not in str(p)
                ]
            if not eval_files:
                continue
            data = _load_json(eval_files[0])
            model = data.get(
                "model",
                _load_config(sibling).get("model", {}).get("name", sibling.name),
            )
            split_data = (
            ...
            test_metrics = split_data.get("metrics", {}) or data.get("summary", {}).get(
                "test", {}
            )
            group_metrics = split_data.get("group_metrics", {})
            if test_metrics:
...
def generate_research_plots(
    ...
    if not eval_paths:
        print(
            "[WARNING] No *_eval_results.json found. Attempting to plot from predictions only."
        )
    ...
    for eval_path in eval_paths:
        ...
        artifact_prefix = (
            f"{artifact_root.name}_" if artifact_root.name in fold_names else ""
        )
        ...
        for split in splits:
            ...
            if split_data.get("group_metrics"):
                _add_metric_row(
                    ...
                    split,
                    "Patient",
                    split_data.get("group_metrics", {}),
                    split_data.get("group_metrics", {}).get("n_groups"),
                )
            ...
            if preds_bundle is not None:
                ...
                for norm, tag in [(True, "normalized"), (False, "raw")]:
                    cm_path = (
                        subdirs["confusion"]
                        / f"{artifact_prefix}confusion_{split}_{tag}.png"
                    )
                    plot_confusion_matrix(
                        targets,
                        preds,
                    ...
                    if cm_path.exists():
                        ...
                        print(
                            f"    [OK] Confusion matrix ({artifact_prefix}{split}, {tag})"
                        )

                # Per-class failure heatmap
                hm_path = (
                    subdirs["class"] / f"{artifact_prefix}failure_heatmap_{split}.png"
                )
                ...
                panel_path = (
                    subdirs["panels"]
                    / f"{artifact_prefix}publication_panel_{split}.png"
                )
            ...
            if (
                ...
                if len(g_tgts) > 0:
                    gcm_path = (
                        subdirs["confusion"]
                        / f"{artifact_prefix}grouped_confusion_{split}_normalized.png"
                    )
                    plot_confusion_matrix(
                        g_tgts,
                        g_preds,
                    ...
                    if gcm_path.exists():
                        ...
                        print(
                            f"    [OK] Grouped confusion matrix ({artifact_prefix}{split})"
                        )
            ...
            if embeddings:
                ...
                if emb_data is not None:
                    ...
                    tsne_path = (
                        subdirs["embeddings"] / f"{artifact_prefix}tsne_{split}.png"
                    )
                    plot_embedding_tsne(
                        features,
                        emb_targets,
        ...
        grp_path = (
            subdirs["grouped"]
            / f"{artifact_prefix}grouped_vs_spectrum_{eval_stage}.png"
        )
        ...
        for metric, stem in [
            ...
            for level, level_stem in [("Spectrum", "spectrum"), ("Patient", "patient")]:
                ...
                if path.exists():
                    ...
                    print(
                        f"    [OK] Fold {level.lower()} {METRIC_DISPLAY[metric]} comparison"
                    )
    ...
    if aggregate_path is not None:
        aggregate = _load_json(aggregate_path)
        if labels_for_aggregate is None:
            labels_for_aggregate = _resolve_labels(
                cfg, _n_classes_from_aggregate(aggregate)
            )
        aggregate_rows = collect_aggregate_metric_rows(aggregate)
        domain_metric_rows = [
            r for r in domain_metric_rows if r.get("fold") != "aggregated"
        ] + aggregate_rows
        figures_generated.extend(
            process_aggregated_cv_results(
                aggregate_path,
                fold_dirs,
